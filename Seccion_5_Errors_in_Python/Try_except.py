print("Seccion 5: Evitar mostrar pila de error. Try/except es mejor para mostrar al usario")

"""
One of Python’s core tenets is: “ask for forgiveness, not for permission”.

Now, I know how well this works with friends and family (hint: not so well), but it works fantastically in Python. 
...
Python suggests that, in many cases, our code can be made more readable by asking for forgiveness instead. 
Doing this (not real Python code):
...
2. Our check for whether it is something we can do is now encapsulated inside the `add_car()` method; 
we don’t need to have an if statement every time we want to add a car.
...
We only do so for errors that match the one in the block (in this case, `TypeError` would be caught, 
other errors would not be caught).
...
"""
Over the next few sections we’ll be making use of this, 
which is why it’s really useful to know how to use `try` and `catch`.

`try` and `catch` also have a final counterpart: `finally`.

We can use `finally` to run a block of code no matter what happens: whether or not an exception is raised. 
For example:
"""
